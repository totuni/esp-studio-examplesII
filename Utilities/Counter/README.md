...
- Source_video: Uses a videocap connector to load video frames from a sample video file. Each frame is treated as an event with an ID and image blob.
...
2. In the right pane, expand **Input Data (Publisher) Connectors**.
...
3. Expand **Subscriber Connectors**. Notice that this window includes a different type of connector. The MQTToutput connector is configured as a subscriber connector. Subscriber connectors output data from a project to an external file or system. 
...
The Source_timed window is designed to emit one event every sixty seconds using the timer connector. This makes it the slowest source in terms of event generation. Therefore, Counter_timed increases very gradually, making it useful for low-rate event simulation and benchmarking long-running processes.
