...
- `REDIS_HOST`: Set this to Redis database server IP or hostname. The default value is **127.0.0.1**.
- `REDIS_PORT`: Set this to Redis database connection port. The default value is **6379**.

For more information, see: ![Properties for Redis connection](img/properties.png "Properties for Redis connection").

## Source Data

To simulate data streaming during testing, a Source window and Timer Connector are used to produce one event per second. Then, a Lua window generates dummy sensor data which consists of the following components:
...
- InputRate is a Source window that ingests dummy events using Timer Connector.
- SensorsData is a Lua window that generates dummy sensor events.
- SaveToRedis is a StateDB Writer window that saves and updates records in Redis hash table.
- GetSavedStamp is a StateDB Reader window that performs lookup by key in  Redis hash table and selects value.
- GetMaxByGroup is a StateDB Reader window that performs aggregate function on Redis hash table by selected group value and selects result.
...
2. In the right pane, expand **Database Write Properties**. Notice the following properties:
   - `Redis prefix`: Allows you to set a unique prefix for the Redis hash table. The default value is **stream**.
   - `Time to live`: Set the retention period in seconds for each key-value record. This is useful for retention-based aggregations. The default value is **60** seconds.
   - `Time field`: Defines the time reference used for retention counting. The default value is **(use system clock)**, you you can change it and specify a timestamp field from the Input event.
   - `Secondary Indexes`: Index SENSOR_GROUP - Fields sensor_group This tables assigns additional indexes on SENSOR_GROUP. The additional field is **sensor_group** by default.
...
4. Click ![edit icon](/Utilities/StateDB/img/edit-icon.png). The following fields are listed:
    ...
    - `sensor_saved_stmp`: This field is mapped to **Query** so it is received from the Redis `SENSOR_STMP` value. 
...
2. In the right pane, expand **Database Query**.
   - `Redis prefix`: Allows you to set a unique prefix for the Redis hash table. The prefix must be the same as the prefix you set in the getSaveStamp window so that it can performa a lookup. The default value is **stream**.
...
4. Click ![edit icon](/Utilities/StateDB/img/edit-icon.png). The following fields are listed:
    ...
    - `sensor_group`: This field is mapped to **Input** so it is received from the Input event.
    - `sensor_stmp`: This field is mapped to **Input** so it is received from the Input event.
    - `sensor_group_max_id`: This field is mapped to **Query** which means it is calculated based on data from the Redis <!-- Redis what? Redis hash table? Redis value? -->. In the example we selected Aggregate function ESP_aMax and  source field `SENSOR_ID`. This means that for each input event for the specific `sensor_group` ('mezzanine','reception' or 'workshop') we will get the maximum `SENSOR_ID` which stored at the moment in Redis, according to 60 seconds retention policy.
...
- The **saveToRedis** tab lists incoming data. The `sensor_id`, `sensor_group`, and `sensor_stmp` fields are stored in a Redis hash. Redis creates indexes on `sensor_id` and `sensor_group` to support future aggregations.
  
  ![saveToRedis window output](img/saveToRedis.png "saveToRedis window output")
- The **getSavedStamp** tab lists incoming data and data from Redis. In the `sensors_saved_stmp` column, you can see fetched data for a specific key (`sensor_id`), which is currently stored in the Redis hash. At the beginning of the test, no data is cached. As the saveToRedis window starts to run, the data appears.
  
  ![getSavedStamp window output](img/getSavedStamp.png "getSavedStamp window output")
- The **getMaxByGroup** tab lists incoming data and data from Redis. In the `sensor_group_max_id` column, you can see aggregated data from Redis - maximum saved `SENSOR_ID` for the `sensor_group` value in the incoming event, where all records limited by 60 seconds TTL.
