...
- Buiding a compute cluster where multiple SAS Event Stream Processing kubernetes pods run as replicas of the same project. The pods run in parallel, sharing and aggregating different parts of the input stream.
...
- GetMaxByGroup is a StateDB Reader window that performs aggregate functions on Redis hash tables by selected group value and selects result.
...
2. In the right pane, expand **Database Write Properties**. Notice the following properties:
   ...
   - `Secondary Indexes`: This tables assigns additional indexes on **SENSOR_GROUP**. The additional field is **sensor_group** by default.
...
2. In the right pane, expand **Database Query**.
   - `Redis prefix`: Allows you to set a unique prefix for the Redis hash table. The prefix must be the same as the prefix you set in the getSavedStamp window so that it can performa a lookup. The default value is **stream**.
...
- The **getMaxByGroup** tab lists incoming data and data from Redis. In the **sensor_group_max_id** column, you can see aggregated data from Redis - maximum saved `SENSOR_ID` for the `sensor_group` value in the incoming event, where all records limited by 60 seconds of retention.
